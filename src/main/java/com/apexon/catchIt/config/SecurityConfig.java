    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity hs) throws Exception {
        hs.csrf(csrf -> csrf.disable())
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers("/registerUser","/getUserById/{id}","/updateUser/{id}","/updatePassword/{id}","/getAllUsers/{adminId}","/updateUserAccountDetails/{adminId}","/assignRoles/{id}","/getUserByName/{uname}").permitAll()
